...
def train_one_epoch(model,tokenizer, dataloader_list,num_batches,num_samples, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    iter_dataloader6 = cycle(iter(dataloader_list[5]))

    for _,i in enumerate(range(num_batches)):
        ...
        elif random.random() < 1/40:
            #1/1000
            batch = next(iter_dataloader4)
        elif random.random() < 1/20:
            #1/1000
            batch = next(iter_dataloader5)
        else:
            batch = next(iter_dataloader6)
