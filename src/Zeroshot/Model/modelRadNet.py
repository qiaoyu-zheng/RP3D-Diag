...
# sys.path.append(".../src/Zeroshot/Utils")
    ...
    def forward(self, image_x, mods, marks, keys, labels):
        ...
        if self.fuse != 'early':
            ...
            if tmp_Aug:
                ...
                # visual_augment(B1, image_x1, ".../src/Zeroshot/Logits/Aug_2D_2")
            ...
            #simple one slice
            # output_x = output_x[torch.arange(B), keys]
            ...
            # score_x = score_x[:,1:]
            # attn_x = attn_x[:,1:]
            # print(attn_x.sum(dim=1))
            # attn_x = attn_x / attn_x.sum(dim=1, keepdim=True)
            # assert torch.all(attn_x.sum(dim=1) - 1 < 1e-5)
            # print("attn_x", attn_x)
            # print("keys", keys)
            # assert torch.all(attn_x > 0)
            # cutoffs = marks.sum(dim=1).to(torch.long)
            # loss_attn = self.attnloss(score_x, keys, cutoffs)

            output_x = self.fc(output_x)
            # print(output_x.shape, labels.shape)
            # loss_cls = self.lossfunc(output_x, labels)
            loss_cls = torch.zeros((B,self.cls)).to(device)
            # loss = loss_attn + loss_cls
            # loss = loss_attn
            # loss_return = loss
            # loss_cls = loss
            return {
            ...
            score_x = score_x[:,1:]
            # score_x = rearrange(score_x, '(b n) d -> b n d', b=bs, n=n, d=self.depth)
